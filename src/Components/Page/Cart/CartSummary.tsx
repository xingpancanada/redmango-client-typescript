...
function CartSummary() {
  ...
  const handleQuantity = (updateQuantityBy: number, cartItem: cartItemModel) => {
    if (updateQuantityBy === 0) {
      ...
      dispatch(removeFromCart({ quantity: 0, cartItem }));
    } else {
      if (cartItem.quantity! + updateQuantityBy <= 0) {
        return;
      } else {
        updateShoppingCart({
          menuItemId: cartItem.menuItem?.id,
        ...
        dispatch(updateQuantity({ quantity: cartItem.quantity! + updateQuantityBy, cartItem }));
      }
    }
  }
  ...
  return (
    <div className="container p-4 m-2">
      ...
      {shoppingCartFromStore.map((cartItem: cartItemModel, index: number) => (  // map(() => ()), not  =>{}
        <div
          key={index}
        ...
        >
          ...
          <div className="p-2 mx-3" style={{ width: "100%" }}>
            ...
            <div className="d-flex justify-content-between">
              ...
              <button className="btn btn-sm btn-danger mx-1" onClick={() => handleQuantity(0, cartItem)} style={{ height: "2rem" }}>Remove</button>
            </div>
          </div>
        </div>
      ))}
